...
@views function diffusion_1D_2procs(; do_visu=false,do_save=false)
    # Physics
    Cl  = 10.0   # left  H
    Cr  = 1.0    # right H
    ...
    if do_save 
        p1=plot(Cg, legend=false, linewidth=0, markershape=:circle, markersize=5, yaxis=font(fontsize, "Courier"), xaxis=font(fontsize, "Courier"), titlefontsize=fontsize, titlefont="Courier")
        p1=plot!(C, legend=false, linewidth=3, framestyle=:box, xlabel="Lx", ylabel="H", title="diffusion (it=$(nt))")
        savefig(p1,"docs/l8ex1t1_2procs.png")
    end    
    return
end

diffusion_1D_2procs(; do_visu=true,do_save=true)
