...
# function callculateing 1D derivative on an array
@views Diff(A) = A[2:end].-A[1:end-1]

@views function diffusion_1D(;ncheck=false)
    # physics
    lx   = 20.0
    dc   = 1.0
    # numerics
...
Res = diffusion_1D(;ncheck=false) # ncheck false to desable visualisation
